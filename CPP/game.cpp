...
int _tmain(int argc, _TCHAR* argv[]) {
  ...
  while(1) {
    ...
    switch (incomingData[incomingName]) {
      case connectedFeather:
        switch (incomingData[1]) {
          ...
          case '9':
            ...
            SP->WriteData(setUntarget);
            Sleep(1000);
            //SP->WriteData(setTarget);
            //serialFeather = SP;
            findNextConnection();
            break;
        };
          ...
          if (isAllHeartbeatsReceived) {
            if (incomingData[1] == '0') {
              if (!isHeartbeatReceived) {
                printf("%c\n", incomingData[1]);
                //serialDue->WriteData(receivedHeartbeat);
                //isHeartbeatReceived = true;
              }
            } else if (incomingData[1] == '1') {
              printf("%c\n", incomingData[1]);
              serialDue->WriteData(receivedConfirmation);
            } else if (incomingData[1] == '2') {
              printf("%c\n", incomingData[1]);
  ...
  while (deviceIndex <= 256) {
    ...
    if (SP->IsConnected()) {
      ...
      if (deviceIndex >= 256) {
        ...
        //isAllHeartbeatsReceived = true;
        //serialDue->WriteData(receivedHeartbeat);
        //printf("ERROR: Arduino Due not found.");
      }
    }
  }
}
