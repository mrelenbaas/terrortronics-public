...
#include <app_framework/Utils/WebRequest.h>
...
WebRequest::WebRequest(struct android_app *app)
  ...
  url_cls_ = reinterpret_cast<jclass>(
      jni_scope->NewGlobalRef(jni_scope->FindClass("java/net/URL")));
  url_ctor_mid_ =
      jni_scope->GetMethodID(url_cls_, "<init>", "(Ljava/lang/String;)V");
  url_open_connection_mid_ = jni_scope->GetMethodID(
      url_cls_, "openConnection", "()Ljava/net/URLConnection;");
  http_url_connection_cls_ = reinterpret_cast<jclass>(
      jni_scope->NewGlobalRef(jni_scope->FindClass("javax/net/ssl/"
                                                   "HttpsURLConnectio"
                                                   "n")));
  set_request_method_mid_ = jni_scope->GetMethodID(
      http_url_connection_cls_, "setRequestMethod", "(Ljava/lang/String;)V");
  set_request_property_mid_ =
      jni_scope->GetMethodID(http_url_connection_cls_, "setRequestProperty",
                             "(Ljava/lang/String;Ljava/lang/String;)V");
  connect_mid_ =
      jni_scope->GetMethodID(http_url_connection_cls_, "connect", "()V");
  set_connect_timout_mid_ = jni_scope->GetMethodID(http_url_connection_cls_,
                                                   "setConnectTimeout", "(I)V");
  set_read_timout_mid_ = jni_scope->GetMethodID(http_url_connection_cls_,
                                                "setReadTimeout", "(I)V");
  set_do_output_mid_ =
      jni_scope->GetMethodID(http_url_connection_cls_, "setDoOutput", "(Z)V");
  get_response_code_mid_ = jni_scope->GetMethodID(http_url_connection_cls_,
                                                  "getResponseCode", "()I");
  get_input_stream_mid_ = jni_scope->GetMethodID(
      http_url_connection_cls_, "getInputStream", "()Ljava/io/InputStream;");
  get_error_stream_mid_ = jni_scope->GetMethodID(
      http_url_connection_cls_, "getErrorStream", "()Ljava/io/InputStream;");
  get_output_stream_mid_ = jni_scope->GetMethodID(
      http_url_connection_cls_, "getOutputStream", "()Ljava/io/OutputStream;");
  input_stream_cls_ = reinterpret_cast<jclass>(
      jni_scope->NewGlobalRef(jni_scope->FindClass("java/io/InputStream")));
  input_stream_read_mid_ =
      jni_scope->GetMethodID(input_stream_cls_, "read", "([BII)I");
  input_stream_close_mid_ =
      jni_scope->GetMethodID(input_stream_cls_, "close", "()V");
  output_stream_cls_ = reinterpret_cast<jclass>(
      jni_scope->NewGlobalRef(jni_scope->FindClass("java/io/OutputStream")));
  output_stream_write_mid_ =
      jni_scope->GetMethodID(output_stream_cls_, "write", "([BII)V");
  output_stream_close_mid_ =
      jni_scope->GetMethodID(output_stream_cls_, "close", "()V");
  get_header_field_mid_ = jni_scope->GetMethodID(
      http_url_connection_cls_, "getHeaderField", "(I)Ljava/lang/String;");
  get_header_field_key_mid_ = jni_scope->GetMethodID(
      http_url_connection_cls_, "getHeaderFieldKey", "(I)Ljava/lang/String;");
}
...
WebRequest::~WebRequest() {
  ...
  if (http_url_connection_cls_)
    jni_scope->DeleteGlobalRef(http_url_connection_cls_);
...
WebResponse WebRequest::request(
    const std::string &url, HTTP_METHOD method,
    const HTTPHeaderList &request_headers,
    std::function<void(const char *data, int size, const void *context)>
        data_available,
  ...
  //  open connection
  jobject connection =
      CALL_OBJECT_METHOD(false, url_obj, url_open_connection_mid_);
  ...
  //  set timeouts
  CALL_VOID_METHOD(false, connection, set_connect_timout_mid_,
                   (jint)connect_timeout_);
  CALL_VOID_METHOD(false, connection, set_read_timout_mid_,
                   (jint)connect_timeout_);
  ...
  if (request_payload != nullptr) {
    ...
    jobject output_stream =
        CALL_OBJECT_METHOD(false, connection, get_output_stream_mid_);
    jbyteArray buffer = jni_scope->NewByteArray(request_payload->buffer_length);
    if (buffer != nullptr) {
      jni_scope->SetByteArrayRegion(buffer, 0, request_payload->buffer_length,
                                    (jbyte *)request_payload->buffer_ptr);
      CALL_VOID_METHOD(false, output_stream, output_stream_write_mid_, buffer,
                       (jint)0, (jint)request_payload->buffer_length);
      CALL_VOID_METHOD(false, output_stream, output_stream_close_mid_);
    } else {
      ALOGE(
          "Failed to allocate JNI byte array of size: %lu for request payload.",
          request_payload->buffer_length);
    }
  }
  ...
  if (data_available) {
    auto getStreamMethodId = response.result >= 400 && response.result <= 599
                                 ? get_error_stream_mid_
                                 : get_input_stream_mid_;
    jobject input_stream =
        CALL_OBJECT_METHOD(false, connection, getStreamMethodId);
    ...
    if (input_stream != nullptr) {
      ...
      do {
        //  read data
        read = CALL_INT_METHOD(false, input_stream, input_stream_read_mid_,
                               buffer, 0, buffer_len);
        if (read > 0) {
  ...
  for (jint i = 0;; i++) {
    //  header field key
    jstring key_str = (jstring)CALL_OBJECT_METHOD(false, connection,
                                                  get_header_field_key_mid_, i);
    if (!key_str) break;

    //  header field value
    jstring val_str = (jstring)CALL_OBJECT_METHOD(false, connection,
                                                  get_header_field_mid_, i);
    if (!val_str) break;
