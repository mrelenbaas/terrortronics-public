...
#define CALL_STATIC_INT_METHOD(is_fatal, ...)  \
  jni_scope->CallStaticIntMethod(__VA_ARGS__); \
  CHECK_FOR_ERRORS(is_fatal, CallStaticObjectMethod)
...
/*!
    ...
    void something( struct android_app *state ) {
        ...
        url_cls_ =
   reinterpret_cast<jclass>(jni_scope->NewGlobalRef(jni_scope->FindClass("java/net/URL")));
        // when jni_scope goes out of scope, the thread is detached
   automatically if it need to be
    }
    \endcode
*/
class ScopedJNIAttach {
 public:
  /*! \brief Operator-> returns JNI Environment Variable. The Java Environment
   * is used to interact with JVM.
   */
  JNIEnv *operator->() {
  ...
  */
  ScopedJNIAttach(JavaVM *vm)
      : jni_env_(nullptr), vm_(vm), should_detach_(false) {
    assert(vm_ != nullptr);
